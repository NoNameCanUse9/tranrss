...
async fn list_apis(
    ...
    let configs = api::list_configs(&state.db)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn get_api(
    ...
    let config = api::get_config(&state.db, id)
        .await
...
async fn update_api(
    ...
    api::update_config(&state.db, id, payload)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
